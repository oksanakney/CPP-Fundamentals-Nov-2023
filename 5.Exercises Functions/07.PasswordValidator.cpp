...
bool validatePassword(string password, bool & lengthValidation, bool & contentValidation, bool & digitValidation) {
    ...
    digitValidation = (digitsNumber >= 2);
    lengthValidation = (password.length() >= 6) && (password.length()) <= 10;
    
    return (lengthValidation && contentValidation && digitValidation);
}
